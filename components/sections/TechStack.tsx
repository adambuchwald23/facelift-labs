"use client";

import { motion } from "motion/react";
import Image from "next/image";
import SectionWrapper from "@/components/ui/SectionWrapper";
import SectionHeader from "@/components/ui/SectionHeader";
import { TECH_STACK_ITEMS } from "@/lib/constants";
import { useIsMobile } from "@/lib/use-mobile";
import { useInView } from "@/lib/use-in-view";
import { staggerContainer } from "@/lib/motion";

const FLOAT_DURATIONS = [3.2, 3.8, 4.1, 3.5, 4.4, 3.0, 3.7];
const SPREAD_X = ["-30px", "-16px", "16px", "30px", "-24px", "0px", "24px"];
const EASE: [number, number, number, number] = [0.22, 1, 0.36, 1];

export default function TechStack() {
  const mobile = useIsMobile();
  const [gridRef, gridInView] = useInView<HTMLDivElement>();
  return (
    <SectionWrapper
      id="tech-stack"
      className="section-viewport px-4 pt-6 pb-8 sm:px-6 sm:py-12 md:pt-10 md:pb-10"
    >
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex justify-center sm:mb-12">
          <SectionHeader label="Tech Stack" />
        </div>

        <motion.div
          ref={gridRef}
          variants={staggerContainer(mobile)}
          initial="hidden"
          animate={gridInView ? "visible" : "hidden"}
          className="flex flex-col items-center gap-3 sm:gap-8"
        >
          <div className="flex flex-wrap justify-center gap-3 sm:gap-8">
            {TECH_STACK_ITEMS.slice(0, 4).map((tool, i) => (
              <Tile key={tool.name} tool={tool} floatIndex={i} mobile={mobile} />
            ))}
          </div>

          <div className="flex flex-wrap justify-center gap-3 sm:gap-8">
            {TECH_STACK_ITEMS.slice(4).map((tool, i) => (
              <Tile key={tool.name} tool={tool} floatIndex={i + 4} mobile={mobile} />
            ))}
          </div>
        </motion.div>
      </div>
    </SectionWrapper>
  );
}

function Tile({
  tool,
  floatIndex,
  mobile,
}: {
  tool: { name: string; logo: string };
  floatIndex: number;
  mobile: boolean;
}) {
  const dur = FLOAT_DURATIONS[floatIndex % FLOAT_DURATIONS.length];
  const spreadX = SPREAD_X[floatIndex % SPREAD_X.length];

  const tileVariants = {
    hidden: { opacity: 0, y: mobile ? 6 : 10, x: mobile ? "0px" : spreadX, scale: 0.96 },
    visible: {
      opacity: 1,
      y: 0,
      x: "0px",
      scale: 1,
      transition: { duration: mobile ? 0.5 : 0.7, ease: EASE },
    },
  };

  return (
    <motion.div variants={tileVariants}>
      <div
        className="tile-float group flex h-[76px] w-[76px] sm:h-[120px] sm:w-[120px] cursor-default flex-col items-center justify-center gap-1.5 sm:gap-3 rounded-[18px] sm:rounded-[28px] bg-[#f5f5f5] ring-1 ring-black/[0.07] transition-shadow duration-300 md:hover:shadow-[0_16px_40px_rgba(0,255,136,0.18),0_4px_12px_rgba(0,0,0,0.08)]"
        style={{
          "--float-dur": `${dur}s`,
          "--float-delay": `${floatIndex * 0.3}s`,
        } as React.CSSProperties}
      >
        <div className="relative h-8 w-8 shrink-0 sm:h-12 sm:w-12">
          <Image
            src={tool.logo}
            alt={`${tool.name} logo`}
            fill
            className="object-contain"
            sizes="56px"
          />
        </div>
        <span className="text-[10px] sm:text-xs font-semibold tracking-wide text-foreground/50 transition-colors group-hover:text-foreground/70">
          {tool.name}
        </span>
      </div>
    </motion.div>
  );
}
